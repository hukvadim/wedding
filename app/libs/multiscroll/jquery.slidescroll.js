...
(function (factory) {
	...
	var Slidescroll = function (element, options) {
		...
		this.isHexaColor = function isHexaColor(sNum){
	...
	Slidescroll.prototype.build = function () {
		...
		this.$pages.each(function (index, item) {
			...
			var colorPin         = $item.data(this.options.namespace+'-active-color');
			...
			if (this.options.generateNavigation) {
				var el = $('<a />');
				var title = ['<span class="slidescroll-pin-index">', pinNum, '</span>'];

				if (titleData || titleSelector) {
					$.merge(title, [
						' <span class="slidescroll-pin-title"><span class="slidescroll-pin-title-text">',
							titleData || $item.find(titleSelector).first().text(),
						'</span></span>',
						' <span class="slidescroll-pin-title-abs">',
							titleData || $item.find(titleSelector).first().text(),
						'</span>'
					]);
				}
				...
				el.html(title.join(''));
				el.addClass('slidescroll-nav-pin');
	...
	Slidescroll.prototype.teardown = function () {
		...
		this.$pages.each(function (index, item) {
			...
			var activeSection = $($blocks[index]).data(this.options.namespace+'-html-class');
			this.removeClass($html, activeSection);
	...
	Slidescroll.prototype.show = function (indexOrKey) {
		var index = this.validIndex(indexOrKey),
			idSection = this.pageKeys[index],
			idOldSection = this.pageKeys[this.current];

		// Add second css class to <html> and remove old section css class
		var activeSection = $($blocks[index]).data(this.options.namespace+'-html-class'),
			oldSection    = $($blocks[this.current]).data(this.options.namespace+'-html-class');

		// Remove current class
		this.removeClass($html, oldSection);
		this.removeClass($html, this.pageKeys[this.current]);
		this.addClass($html,    activeSection);
